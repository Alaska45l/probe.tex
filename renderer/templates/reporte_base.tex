...
\begin{tabularx}{\linewidth}{@{} X c c c c @{}}
  ...
  Stress combinado (30 s)      & \mv{<< gpu_vram_stress_gb >>\ GB} & Mixed &
...
\begin{componentinfo}
  ...
  \textbf{FW:}\ << nvme_firmware >>\\
  \textbf{TBW restante:}\ \mv{<< nvme_tbw_remaining >>\ TB}\ de\ << nvme_tbw_rated >>\ TB\hfill
  \textbf{Horas de uso:}\ \mv{<< nvme_hours >>\ h}
\end{componentinfo}
...
\begin{tabularx}{\linewidth}{@{} X r r c @{}}
  ...
  \midrule
  Write Amplification Factor (WAF) & \mv{<< nvme_waf >>} & $\leq 3.0$ &
    [% if nvme_waf_ok %]\badgeok[% else %]\badgewarn[% endif %] \\
  \rowalt
  LBA escritos (host)         & \mv{<< nvme_lba_written >>\ TB}  & --- & --- \\
  NAND escritas (total)       & \mv{<< nvme_nand_written >>\ TB} & --- & --- \\
  \rowalt
...
\begin{minipage}[t]{0.54\linewidth}
  ...
  \vspace{0.4cm}
  \small
  \textbf{Velocidad efectiva:}\ \mv{<< ram_speed_effective >>\ MHz}\\
  \textbf{Latencia CAS medida:}\ \mv{<< ram_cas_ns >>\ ns}\\
...
\textbf{Temperatura VRM:}\ \mv{<< vrm_temp >>$^\circ$C}\quad
\textbf{Fases VRM:}\ \mv{<< vrm_phases >>}\quad
