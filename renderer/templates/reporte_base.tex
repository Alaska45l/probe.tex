...
% ════════════════════════════════════════════════════════════════════════════
%  SECCIÓN 3 --- ALMACENAMIENTO NO VOLÁTIL (SSD/NVMe o HDD Mecánico)
%
%  NOTA DE INTEGRACIÓN:
%  Reemplazar TODA la Sección 3 original en reporte_base.tex por este bloque.
...
\begin{componentinfo}
  \small
  \textbf{Dispositivo:}\ \texttt{<< nvme_device >>}\hfill
  \textbf{Modelo:}\ \mv{<< nvme_model >>}\hfill
  \textbf{Capacidad:}\ \mv{<< nvme_capacity >>\ GB}\hfill
  \textbf{FW:}\ << nvme_firmware >>\hfill
  \textbf{Horas de uso:}\ \mv{<< nvme_hours >>\ h}\hfill
  \textbf{Tipo:}\ [% if is_hdd %]\mv{HDD Mecánico}[% else %]\mv{SSD/NVMe}[% endif %]
\end{componentinfo}
...
\begin{minipage}[t]{0.56\linewidth}
  ...
  \begin{tabularx}{\linewidth}{@{} X r c @{}}
    ...
    \midrule
    % ── Spin-Up Time (SMART ID 3) ─────────────────────────────────────
    Spin-Up Time (ID 3) &
      [% if hdd_spin_up_time is not none %]%
        \mv{<< hdd_spin_up_time >>\ ms}%
      [% else %]%
        \textcolor{AMSlate}{\textit{N/D}}%
      [% endif %] & --- \\
    \rowalt
    % ── Seek Latency (fio randread 4K QD1) ───────────────────────────
    Seek Latency media (fio) &
      [% if hdd_seek_latency_ms is not none %]%
        \mv{<< hdd_seek_latency_ms >>\ ms}%
      [% else %]%
        \textcolor{AMSlate}{\textit{N/D}}%
      [% endif %] &
      [% if hdd_seek_latency_ms is not none %]%
        [% if hdd_seek_latency_ms > 25.0 %]\badgewarn[% else %]\badgeok[% endif %]%
      [% else %]%
        \badgeinfo%
      [% endif %] \\
    % ── Reallocated Sector Count (SMART ID 5) ────────────────────────
    Sectores Reasignados (ID 5) &
      [% if hdd_reallocated_sectors is not none %]%
        \mv{<< hdd_reallocated_sectors >>}%
      [% else %]%
        \textcolor{AMSlate}{\textit{N/D}}%
      [% endif %] &
      [% if hdd_reallocated_sectors is not none %]%
        [% if hdd_reallocated_sectors > 0 %]\badgefail[% else %]\badgeok[% endif %]%
      [% else %]%
        \badgeinfo%
      [% endif %] \\
    \rowalt
    % ── Command Timeout (SMART ID 188) ───────────────────────────────
    Command Timeouts (ID 188) &
      [% if hdd_command_timeouts is not none %]%
        \mv{<< hdd_command_timeouts >>}%
      [% else %]%
        \textcolor{AMSlate}{\textit{N/D}}%
      [% endif %] &
      [% if hdd_command_timeouts is not none %]%
        [% if hdd_command_timeouts > 0 %]\badgefail[% else %]\badgeok[% endif %]%
      [% else %]%
        \badgeinfo%
      [% endif %] \\
    \bottomrule
  ...
  \vspace{0.25cm}
  {\scriptsize\color{AMSlate}
   Seek Latency medida con \texttt{fio randread 4K QD1, runtime=10s}.
   Umbral de degradación: $>$25 ms.}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.41\linewidth}
  \vspace{0pt}
  % ── Bloque de estado condicional ──────────────────────────────────
  [% if (hdd_reallocated_sectors is not none and hdd_reallocated_sectors > 0)
     or (hdd_command_timeouts is not none and hdd_command_timeouts > 0) %]
  \begin{amboxcrit}{Daño Físico / Fallo de Actuador}
    \scriptsize
    [% if hdd_reallocated_sectors is not none and hdd_reallocated_sectors > 0 %]
    \textbf{<< hdd_reallocated_sectors >>} sector(es) reasignado(s).
    Daño físico confirmado en el plato.\\[3pt]
    [% endif %]
    [% if hdd_command_timeouts is not none and hdd_command_timeouts > 0 %]
    \textbf{<< hdd_command_timeouts >>} timeout(s) de comando (ID 188).
    Inestabilidad de actuador.\\[3pt]
    [% endif %]
    \textbf{Backup inmediato mandatorio.}
  \end{amboxcrit}
  [% elif hdd_seek_latency_ms is not none and hdd_seek_latency_ms > 25.0 %]
  \begin{amboxwarn}{Degradación Cinemática}
    \scriptsize
    Seek latency: \mv{<< hdd_seek_latency_ms >>\ ms}\\
    (umbral: 25 ms).\\[3pt]
    El cabezal pierde agilidad mecánica.\\
    Planificar reemplazo preventivo.
  \end{amboxwarn}
  [% else %]
  \begin{amboxok}{Cinemática Mecánica Normal}
    \scriptsize
    Sin sectores reasignados ni timeouts.\\
    [% if hdd_seek_latency_ms is not none %]
    Seek latency: \mv{<< hdd_seek_latency_ms >>\ ms} --- dentro del rango nominal.
    [% else %]
    Seek latency no medida (fio no disponible).
    [% endif %]
  \end{amboxok}
  [% endif %]
\end{minipage}
...
\begin{tabularx}{\linewidth}{@{} X r @{}}
  ...
  \midrule
  Horas de encendido (Power-On Hours)  & \mv{<< nvme_hours >>\ h} \\
  \rowalt
  Spin-Up Time (ID 3, raw)             &
    [% if hdd_spin_up_time is not none %]\mv{<< hdd_spin_up_time >>\ ms}[% else %]\textcolor{AMSlate}{\textit{N/D}}[% endif %] \\
  Sectores Reasignados (ID 5)          &
    [% if hdd_reallocated_sectors is not none %]\mv{<< hdd_reallocated_sectors >>}[% else %]\textcolor{AMSlate}{\textit{N/D}}[% endif %] \\
  \rowalt
  Command Timeouts (ID 188)            &
    [% if hdd_command_timeouts is not none %]\mv{<< hdd_command_timeouts >>}[% else %]\textcolor{AMSlate}{\textit{N/D}}[% endif %] \\
  Seek Latency media (fio, 10 s)       &
    [% if hdd_seek_latency_ms is not none %]\mv{<< hdd_seek_latency_ms >>\ ms}[% else %]\textcolor{AMSlate}{\textit{N/D --- instalar fio}}[% endif %] \\
  \bottomrule
\end{tabularx}

[% else %]
% ────────────────────────────────────────────────────────────────────────────
%  RAMA SSD/NVMe: Latencia I/O y Desgaste NAND (sin cambios respecto a v1.0)
% ────────────────────────────────────────────────────────────────────────────

\begin{componentinfo}
  \small
  \textbf{TBW restante:}\ \mv{<< nvme_tbw_remaining >>\ TB}\ de\ << nvme_tbw_rated >>\ TB
\end{componentinfo}
...
\begin{axis}[
  ...
  \addplot[
    ...
    (b0,[% if lat_b0 == 0 %]1[% else %]<< lat_b0 >>[% endif %])
    (b1,[% if lat_b1 == 0 %]1[% else %]<< lat_b1 >>[% endif %])
    (b2,[% if lat_b2 == 0 %]1[% else %]<< lat_b2 >>[% endif %])
    (b3,[% if lat_b3 == 0 %]1[% else %]<< lat_b3 >>[% endif %])
    (b4,[% if lat_b4 == 0 %]1[% else %]<< lat_b4 >>[% endif %])
    (b5,[% if lat_b5 == 0 %]1[% else %]<< lat_b5 >>[% endif %])
    (b6,[% if lat_b6 == 0 %]1[% else %]<< lat_b6 >>[% endif %])
    (b7,[% if lat_b7 == 0 %]1[% else %]<< lat_b7 >>[% endif %])
    (b8,[% if lat_b8 == 0 %]1[% else %]<< lat_b8 >>[% endif %])
    (b9,[% if lat_b9 == 0 %]1[% else %]<< lat_b9 >>[% endif %])
  };
...
\noindent\small
\textbf{P50:}\ \mv{<< nvme_lat_p50 >>\ $\mu$s}\quad
\textbf{P95:}\ \mv{<< nvme_lat_p95 >>\ $\mu$s}\quad
\textbf{P99:}\ \mv{<< nvme_lat_p99 >>\ $\mu$s}\quad
\textbf{P99.9:}\ \mv{<< nvme_lat_p999 >>\ $\mu$s}
...
\begin{tabularx}{\linewidth}{@{} X r r c @{}}
  ...
  Write Amplification Factor (WAF) &
    [% if nvme_waf > 0.0 %]
    \mv{<< nvme_waf >>} & $\leq 3.0$ & [% if nvme_waf_ok %]\badgeok[% else %]\badgewarn[% endif %]
    [% else %]
    \mv{N/A (no expuesto por firmware)} & --- & \badgeinfo
    [% endif %] \\
  \rowalt
  LBA escritos (host)         & \mv{<< nvme_lba_written >>\ TB}  & --- & --- \\
  NAND escritas (total)       &
    [% if nvme_nand_written > 0.0 %]
    \mv{<< nvme_nand_written >>\ TB}
    [% else %]
    \textcolor{AMSlate}{\textit{N/D}}
    [% endif %] & --- & --- \\
  \rowalt
  Bloques NAND defectuosos    & \mv{<< nvme_bad_blocks >>}   & $\leq 50$ &
    [% if nvme_bad_blocks_ok %]\badgeok[% else %]\badgefail[% endif %] \\
  Spare blocks restantes      & \mv{<< nvme_spare_blocks >>} & $\geq 10$ &
    [% if nvme_spare_ok %]\badgeok[% else %]\badgewarn[% endif %] \\
  \rowalt
  Errores ECC corregibles     & \mv{<< nvme_ecc_errors >>}   & $< 100$ &
    [% if nvme_ecc_ok %]\badgeok[% else %]\badgewarn[% endif %] \\
  Temperatura máx. registrada & \mv{<< nvme_t_max >>$^\circ$C} & $< 70^\circ$C &
    [% if nvme_temp_ok %]\badgeok[% else %]\badgewarn[% endif %] \\
  \rowalt
  Porcentaje de vida restante & \mv{<< nvme_life_pct >>\%} & $\geq 20\%$ &
    [% if nvme_life_ok %]\badgeok[% else %]\badgewarn[% endif %] \\
  \bottomrule
\end{tabularx}

[% endif %]
% ─── FIN DE LA BIFURCACIÓN HDD / SSD ────────────────────────────────────────
...
\begin{tabularx}{\linewidth}{@{} X c c l @{}}
  ...
  Almacenamiento NVMe/SSD       & \textbf{+<< nvme_anomalia >>} & << nvme_estado_badge >> & --- \\
  \rowalt
